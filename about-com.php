<?php
  ...
  $view_sql = "SELECT * FROM `chat` WHERE client='$email' ORDER BY time ASC";
  ...
  if (isset($_POST['submit'])) {
      $ms = $_POST['ms'];
      $sql = "INSERT INTO chat(messages,send,client) VALUES('$ms',1,'$email')";
      $support_on = "UPDATE clients SET support=1 WHERE email='$email'";
      $query = mysqli_query($apple->conn,$sql);
      mysqli_query($apple->conn,$support_on);
  }

?>


<h4 style="display:inline-block ;">Chat with support</h4> <a href="">Refresh</a>
<hr>
<section class="msger">
  <main id="scroll-to-bottom" class="msger-chat" style=" background:#ddd; overflow:scroll; height:450px;">
    ...
    <?php while($info = mysqli_fetch_array($all_message)){ ?>
      <?php 
        $read_id = $info['id'];
        //echo $read_id;
        $read_msg = "UPDATE chat SET read_stat = 1 WHERE id='$read_id' AND send=0";
        $query = mysqli_query($apple->conn,$read_msg);
      ?>   
      <?php if($info['send']==1){ ?>
      <div class="msg left-msg">
        <div class="msg-bubble">
          <div class="msg-info">
            <div class="msg-info-name"><?php echo $firstname," ",$lastname ?></div>
  ...
  <form class="msger-inputarea" method="post" action="?p=about">
...
<!-- <div>
  ...
  <hr>
  <div class="enter-message grid-container-element">
    <div class="grid-child-element">
      <input type="text" class="input-msg">
    </div>
    <div class="grid-child-element send-btn">
      <button>Send</button>
    </div>
  </div>
</div>
